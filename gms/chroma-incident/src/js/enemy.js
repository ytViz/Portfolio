function Enemy(x,y, health, size=8, color=0) {
    ...
    this.draw = function(){
        ...
        if(inView(sx,sy,24)){
            ...
            // circle(sx,sy,2,0,0);
            pat=dither[8];
            fillEllipse(sx,sy-ymod,this.width,this.height,0,64);
            ellipse(sx,sy-ymod,this.width,this.height,this.color,this.color-1);
            //eyes
            let inc = this.width/5|0;
            setColors(22,22);
            if(t%40 > this.blinkTimer){
                fillEllipse(sx+inc,sy+inc,inc,inc,22,22);
                fillEllipse(sx+inc*3,sy+inc,inc,inc,22,22);
            } 
